document.addEventListener("DOMContentLoaded", () => {
  ...
  function updateSummary() {
    ...
    const annualIncomeAmount = transactions
      .filter(
        (t) =>
          t.type === "income" && new Date(t.date).getFullYear() === currentYear
      )
      .reduce((sum, t) => sum + t.amount, 0);
    const annualExpensesAmount = transactions
      .filter(
        (t) =>
    ...
    const monthlyIncomeAmount = transactions
      .filter(
        (t) =>
          t.type === "income" &&
      ...
      .reduce((sum, t) => sum + t.amount, 0);
    const monthlyExpensesAmount = transactions
      .filter(
        (t) =>
          t.type === "expense" &&
          new Date(t.date).getFullYear() === currentYear &&
    ...
    transactionsDiv.innerHTML = transactions
      .map(
        (t) => `<p>${t.date} - ${t.description}: ${t.amount} (${t.type})</p>`
      )
      .join("");
  }
  ...
  function handleChatMessage(message) {
    ...
    if (message.toLowerCase().includes("income")) {
      ...
      response =
        "To manage expenses, you should track your spending and create a budget.";
